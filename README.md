...
```
   ...
   │  nginx + MVC           │ ◀┄┄┄┄ │  Spring Boot           │ ◀┄┄┄┄ │  repositorios,         │
   │  (model/view/control.) │        │  servicios y reglas    │        │  entidades, @Transact. │
   └────────────────────────┘ FRONT. └────────────────────────┘        └───────────┬────────────┘
                               REMOTA 1                                            │ JDBC
                                                                       FRONTERA    │ REMOTA 2
                                                                                   ▼
...
- El **tier de presentación** es un servicio desplegado, no el navegador del usuario:
  sirve la aplicación MVC y hace de proxy inverso de `/api`, de modo que el navegador
  ve un solo origen y no hay CORS.
...
```
frontend/                 Tier de presentación — MVC en módulos ES nativos
  src/model/                estado y acceso a datos (no toca el DOM)
  src/view/                 acceso al DOM (no llama a la API)
  src/controller/           coordina modelo y vista
